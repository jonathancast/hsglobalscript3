...
import GSI.Value (GSValue(..), GSError(..), GSException(..), GSEvalState(..), gsundefined_value_w, gsapply_w, gslambda_w, gsthunk_w, gsargexpr_w, gsimpfor_w, gsvCode)
import GSI.Eval (GSResult(..), eval, evalSync, stCode)
import GSI.ByteCode (gsbcrehere_w, gsbcundefined_w, gsbcimpbody_w)
...
main = runTestTT $ TestList $ [
    ...
    TestCase $ do
        ...
        let col = 1
        op <- bitBucketOPort
        v <- evalSync (GSEvalState op Nothing) [] =<< getThunk =<< gsapply_w (Pos file line col) (gsundefined_value_w (Pos file line col)) []
        case v of
    ...
    TestCase $ do
        ...
        fn <- gsthunk_w (Pos file 2 1) $ gsbcundefined_w (Pos file 3 1)
        op <- bitBucketOPort
        v <- evalSync (GSEvalState op Nothing) [] =<< getThunk =<< gsapply_w (Pos file 1 1) fn [gsundefined_value_w (Pos file 4 1)]
        case v of
            GSImplementationFailure pos msg -> assertFailure $ fmtPos pos $ msg
    ...
    TestCase $ do
        let file = "test-file.gs"
        op <- bitBucketOPort
        v <- evalSync (GSEvalState op Nothing) [] =<< getThunk =<< gsapply_w (Pos file 1 1) (gslambda_w (Pos file 2 1) $ (\ (x :: GSValue) -> gsbcundefined_w (Pos file 3 1))) [gsundefined_value_w (Pos file 4 1)]
        case v of
            GSImplementationFailure pos msg -> assertFailure $ fmtPos pos $ msg
            GSError (GSErrUnimpl (StackTrace pos _)) -> assertEqual "The returned error has the right location" pos (Pos file 3 1)
            _ -> assertFailure $ "Got " ++ gsvCode v ++ "; expected error"
    ...
    TestCase $ do
        let file = "test-file.gs"
        op <- bitBucketOPort
        v <- evalSync (GSEvalState op Nothing)  [] =<< getThunk =<< gsapply_w (Pos file 1 1) (gslambda_w (Pos file 2 1) $ (\ (x :: GSValue) -> gsbcrehere_w (Pos file 3 1) $ gsbcundefined_w (Pos file 4 1))) [gsundefined_value_w (Pos file 4 1)]
        case v of
            GSImplementationFailure pos msg -> assertFailure $ fmtPos pos msg
            GSError (GSErrUnimpl (StackTrace pos _)) -> assertEqual "The returned error has the right location" pos (Pos file 3 1)
            _ -> assertFailure $ "Got " ++ gsvCode v ++ "; expected error"
    ...
    TestCase $ do
        ...
        let line = 1
        op <- bitBucketOPort
        t <- createThread (GSEvalState op Nothing) $gshere (gsundefined_value_w (Pos file line 1)) Nothing
        return ()
    ,
    TestCase $ do
        let file = "test-file.gs"

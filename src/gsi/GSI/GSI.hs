...
import GSI.Value (GSValue(..), GSExpr, GSEvalState(..), GSIntArg(..), GSIntExpr(..), GSBCO(..), GSEvalState(..), GSExternal(..), GSException(..), Thread, gslambda_value, gsconstr, gsrecord, gsimpprim, gsthunk_w, gsintthunk_w, gsapply_w, gsundefined_value_w, gsexternal, gsav, gsae, gsargexpr_w, gsvFmt, gsvCode, whichExternal)
import GSI.Thread (createThread, execMainThread)
import API (apiImplementationFailure)
import GSI.Eval (evalSync)
import GSI.Functions (gslist, gsstring, gsapiEvalPos, gsapiEvalExternal, gsapiEvalList, fmtError)
...
gsigsthunk = $gsimpprim $ \ evs pos0 th (pos1v :: GSValue) (ev :: GSValue) -> do
    pos1 <- gsapiEvalPos evs pos0 pos1v
    e <- gsapiEvalExternal evs pos0 ev :: IO GSExpr
    r <- gsthunk_w pos1 e
    return $ gsexternal r

gsigsintthunk = $gsimpprim $ \ evs pos0 th (pos1v :: GSValue) (ev :: GSValue) -> do
    pos1 <- gsapiEvalPos evs pos0 pos1v
    e <- gsapiEvalExternal evs pos0 ev :: IO GSIntExpr
...
gsiprimgsapply evs pos t fv asv = do
    f <- gsapiEvalExternal evs pos fv :: IO GSValue
    as <- (gsapiEvalList evs pos asv >>= mapM (\ av -> gsapiEvalExternal evs pos av >>= \ a -> return a)) :: IO [GSValue]
...
gsiprimexecMainThread evs pos tself tprogv = do
    tprog <- gsapiEvalExternal evs pos tprogv
...
gsigsfmtError = $gslambda_value $ \ e -> $gsbcevalexternal ($gsav e) $ \ hse -> $gsbcimpprim $ \ evs pos t ->
    do es <- fmtError hse; return $ $gsstring es

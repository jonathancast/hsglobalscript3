...
gsbcundefined_w pos = GSExpr $ \ msg pc cs sk -> gsthrow sk $ GSError $ GSErrUnimpl $ StackTrace pos cs

gsbcrehere_w :: Pos -> GSExpr -> GSExpr
gsbcrehere_w pos e = GSExpr $ \ msg pc cs sk -> runGSExpr e msg pc cs $ aceReHere pos cs sk

gsbcat_w :: Pos -> [StackTrace] -> GSExpr -> GSExpr
gsbcat_w pos cs k = GSExpr $ \ msg pc _ sk -> runGSExpr k msg pc cs sk

gsbcrune = varE 'gsbcrune_w `appE` gshere

gsbcrune_w :: Pos -> Char -> GSExpr
gsbcrune_w pos r = GSExpr $ \ msg pc cs sk -> gsreturn sk $ GSRune r

gsbcnatural = varE 'gsbcnatural_w `appE` gshere

gsbcnatural_w :: Pos -> Integer -> GSExpr
gsbcnatural_w pos n = GSExpr $ \ msg pc cs sk -> gsreturn sk $ GSNatural n

gsbcrecord = varE 'gsbcrecord_w `appE` gshere

gsbcrecord_w :: Pos -> [(GSVar, GSArg)] -> GSExpr
gsbcrecord_w pos fs = GSExpr $ \ msg pc cs sk -> do
    fs' <- forM fs $ \ (v, fa) -> do
...
gsbcconstr_w pos c as = GSExpr $ \ msg pc cs sk -> do
...
gsbcexternal_w pos e = GSExpr $ \ msg pc cs sk -> gsreturn sk $ GSExternal (toExternal e)

gsbcchar_w :: Pos -> Char -> GSExpr
gsbcchar_w pos ch = GSExpr $ \ msg pc cs sk -> gsreturn sk $ GSRune ch

gsbcerror = varE 'gsbcerror_w `appE` gshere

gsbcerror_w :: Pos -> String -> GSExpr
gsbcerror_w pos msg = GSExpr $ \ _ _ _ sk -> gsthrow sk $ GSError (GSErrError pos msg)

gsbcruntimetypeerror = varE 'gsbcruntimetypeerror_w `appE` gshere
gsbcruntimetypeerror_w :: Pos -> String -> String -> String -> GSExpr
gsbcruntimetypeerror_w pos ctxt act exp = GSExpr $ \ msg pc cs sk -> gsthrow sk $ GSInvalidProgram $ GSIPRuntimeTypeError (StackTrace pos cs) ctxt act exp

gsbcimplementationfailure = varE 'gsbcimplementationfailure_w `appE` gshere

gsbcimplementationfailure_w :: Pos -> String -> GSExpr
gsbcimplementationfailure_w pos msgs = GSExpr $ \ msg pc cs sk -> gsthrow sk $ GSImplementationFailure pos msgs

gsbcarg = varE 'gsbcarg_w `appE` gshere

gsbcarg_w :: Pos -> (GSValue -> GSExpr) -> GSExpr
gsbcarg_w pos fn = GSExpr $ \ msg pc cs sk -> do
...
gsbcenter_w pos v = GSExpr $ \ msg pc cs sk -> aceEnter msg pc [ StackTrace pos cs ] v sk

gsbcenterarg = varE 'gsbcenterarg_w `appE` gshere

gsbcenterarg_w :: Pos -> GSArg -> GSExpr
gsbcenterarg_w pos (GSArgExpr pos1 e) = GSExpr $ \ msg pc cs sk -> runGSExpr e msg pc [ StackTrace pos1 [ StackTrace pos cs ] ] sk
...
gsbcapply_w pos f args = GSExpr $ \ msg pc cs sk -> do
...
gsbcapp_w pos f args = GSExpr $ \ msg pc cs sk -> do
    asv <- mapM (gsprepare_w pos) args
...
instance GSBCPrimType (IO GSValue) GSExpr where
    gsbcprim_ww pos f = GSExpr $ \ msg pc cs sk -> do
        v <- f msg pc
...
gsbcforce_w pos e k = GSExpr $ \ msg pc cs sk -> let c1 = StackTrace pos cs in runGSArg msg pc c1 e (aceForce msg pc cs k sk)
...
gsbclet_w pos e k = GSExpr $ \ msg pc cs sk -> do
    v <- gsthunk_w pos e
...
gsbcimpfor_w pos a = GSExpr $ \ msg pc cs sk -> gsreturn sk $ gsimpfor_w pos a

gsbclfield = varE 'gsbclfield_w `appE` gshere

gsbclfield_w :: Pos -> GSVar -> GSValue -> (GSValue -> GSExpr) -> GSExpr
gsbclfield_w pos f r k = GSExpr $ \ msg pc cs sk -> do
    v <- gsfield_w pos f r
    runGSExpr (k v) msg pc [StackTrace pos cs] sk
...
gsbcfield_w pos a f = GSExpr $ \ msg pc cs sk -> let c1 = StackTrace pos cs in runGSArg msg pc c1 a (aceField msg pc c1 f sk)
...
gsbcfmterrormsg_w pos msga k = GSExpr $ \ msg pc cs sk -> do
...
gsbcvarpattern_w pos v = gsbcarg_w pos $ \ x -> GSExpr $ \ msg pc cs sk ->
...
gsbcdiscardpattern_w pos = gsbcarg_w pos $ \ x -> GSExpr $ \ msg pc cs sk ->
